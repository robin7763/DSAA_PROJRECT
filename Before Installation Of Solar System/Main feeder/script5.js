...
const workbook = xlsx.readFile('./Book5.xlsx')
...
const voltage_i = math.complex(loads[loads.length - 1].Re_Node_voltages_iteration_2 / 1000, loads[loads.length - 1].Imz_Node_voltages_iteration_2 / 1000);


let current_i = math.divide(kva_i, voltage_i);
current_i = current_i.conjugate()

loads[loads.length - 1].Re_line_current_iteration_2 = math.re(current_i)
loads[loads.length - 1].Imz_line_current_iteration_2 = math.im(current_i)



for (let i = loads.length - 1; i >= 2; i--) {

    const kva_i_1 = math.complex(loads[i - 1].realLoad, loads[i - 1].reactiveLoad);
    const voltage_i_1 = math.complex(loads[i - 1].Re_Node_voltages_iteration_2 / 1000, loads[i - 1].Imz_Node_voltages_iteration_2 / 1000);
    ...
    if (i == loads.length - 1) {
        ...
        loads[i - 1].Re_line_current_iteration_2 = math.re(current_i_1)
        loads[i - 1].Imz_line_current_iteration_2 = math.im(current_i_1)
    ...
    loads[i - 2].Re_line_current_iteration_2 = math.re(current_i_2)
    loads[i - 2].Imz_line_current_iteration_2 = math.im(current_i_2)
...
xlsx.writeFile(newWorkbook, 'Book6.xlsx')
